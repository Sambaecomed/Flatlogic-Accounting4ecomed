...
module.exports = class JournalsDBApi {
  ...
  static async bulkImport(data, options) {
    ...
    // Bulk create items
    const journals = await db.journals.bulkCreate(journalsData, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    const journals = await db.journals.findOne({ where }, { transaction });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.name) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('journals', 'name', filter.name),
        };
      }
    ...
    let { rows, count } = options?.countOnly
      ...
      : await db.journals.findAndCountAll({
          where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
