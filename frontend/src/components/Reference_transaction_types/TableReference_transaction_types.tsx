...
import { loadColumns } from './configureReference_transaction_typesCols';
...
const TableSampleReference_transaction_types = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: reference_transaction_typesNotify,
    refetch,
  } = useAppSelector((state) => state.reference_transaction_types);
  ...
  useEffect(() => {
    if (reference_transaction_typesNotify.showNotification) {
      notify(
        reference_transaction_typesNotify.typeNotification,
        reference_transaction_typesNotify.textNotification,
      );
    }
  }, [reference_transaction_typesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/reference_transaction_types/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(
      `/reference_transaction_types/reference_transaction_types-view/?id=${id}`,
    );
  };
    ...
    <>
      ...
      <div className='relative overflow-x-auto'>
        <DataGrid
          ...
          rows={reference_transaction_types ?? []}
          columns={columns}
          initialState={{
            pagination: {
              paginationModel: {
...
export default TableSampleReference_transaction_types;
