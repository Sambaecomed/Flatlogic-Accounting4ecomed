...
import { loadColumns } from './configureAccountgroupCols';
...
const TableSampleAccountgroup = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: accountgroupNotify,
    refetch,
  } = useAppSelector((state) => state.accountgroup);
  ...
  useEffect(() => {
    if (accountgroupNotify.showNotification) {
      notify(
        accountgroupNotify.typeNotification,
        accountgroupNotify.textNotification,
      );
    }
  }, [accountgroupNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/accountgroup/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/accountgroup/accountgroup-view/?id=${id}`);
  };
...
export default TableSampleAccountgroup;
