...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/general_ledger/general_ledgerSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureGeneral_ledgerCols';
...
const TableSampleGeneral_ledger = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: general_ledgerNotify,
    refetch,
  } = useAppSelector((state) => state.general_ledger);
  ...
  useEffect(() => {
    if (general_ledgerNotify.showNotification) {
      notify(
        general_ledgerNotify.typeNotification,
        general_ledgerNotify.textNotification,
      );
    }
  }, [general_ledgerNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/general_ledger/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/general_ledger/general_ledger-view/?id=${id}`);
  };
...
export default TableSampleGeneral_ledger;
